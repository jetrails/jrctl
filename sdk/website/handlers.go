...
func SwitchPHP(context config.Context, data PhpSwitchRequest) PhpSwitchResponse {
	var request = gorequest.New()
	request.SetDebug(context.Debug)
	request.TLSClientConfig(&tls.Config{InsecureSkipVerify: context.Insecure})
	_, body, errs := request.
	...
	var response PhpSwitchResponse
	json.Unmarshal([]byte(body), &response)
	return response
}

func AvailablePHP(context config.Context) PhpAvailableResponse {
	var request = gorequest.New()
	request.SetDebug(context.Debug)
	request.TLSClientConfig(&tls.Config{InsecureSkipVerify: context.Insecure})
	_, body, errs := request.
		Timeout(10*time.Second).
		Get(fmt.Sprintf("https://%s/website/php", context.Endpoint)).
		Set("Content-Type", "application/json").
		Set("User-Agent", fmt.Sprintf("jrctl/%s", version.VersionString)).
		Set("Authorization", "Bearer "+context.Token).
		Type("text").
	...
	var response PhpAvailableResponse
	json.Unmarshal([]byte(body), &response)
	return response
}
